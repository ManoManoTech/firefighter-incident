...
class SlackUserManager(models.Manager["SlackUser"]):
    ...
    def get_user_by_slack_id(
        ...
        # If not in DB, fetch the user's info from Slack...
        logger.debug("Fetch user from Slack")
        try:
            user_info = client.users_info(user=slack_id)
            if not user_info.get("ok"):
                logger.error("Could not fetch user from Slack.")
                return None
    ...
    def add_slack_id_to_user(
        ...
        if not user_info.get("ok"):
            logger.error(f"Could not fetch user from Slack. User: email={user.email}")
            return None
...
class SlackUser(models.Model):
    ...
    def update_user_info(
        ...
        try:
            ...
            if not user_info.get("ok"):
                logger.warning("Could not fetch user from Slack.")
                return
        except slack_sdk.errors.SlackApiError:
