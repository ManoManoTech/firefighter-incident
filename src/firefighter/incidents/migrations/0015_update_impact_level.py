...
IMPACT_LEVEL_NAME_MAPPING = {
    ...
    ("NO", "business_impact"): ("No impact on business.", ""),
    ("LT", "customers_impact"): (
    ...
    ("NO", "customers_impact"): ("No impact for customers.", ""),
    ("LT", "sellers_impact"): (
    ...
    ("NO", "sellers_impact"): ("No impact for sellers.", ""),
    ("LT", "employees_impact"): (
    ...
    ("NO", "employees_impact"): ("No impact for employee.", ""),
}
...
def update_impact_level_names(apps, _schema_editor):
    ...
    for (value, impact_type_value), (
        new_name,
        description,
    ) in IMPACT_LEVEL_NAME_MAPPING.items():
        try:
            impact_type = ImpactType.objects.filter(value=impact_type_value).first()
            if not impact_type:
                logger.warning(
                    f"ImpactType '{impact_type}' does not exist. Skipping update for value '{value}' and impact_type '{impact_type_value}'."
                )
                continue

            impact_level = ImpactLevel.objects.get(
                value=value, impact_type=impact_type.id
            )
            impact_level.name = new_name
            impact_level.description = description
            impact_level.save()
            logger.info(
                f"Updated ImpactLevel ({value}, {impact_type}) to name '{new_name}'"
            )
        except ImpactLevel.DoesNotExist:
            logger.warning(
                f"ImpactLevel with value '{value}' and impact_type '{impact_type}' does not exist."
            )
