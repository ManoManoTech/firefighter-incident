...
COMPONENT_SLACK_GROUPS = {
    ...
    "Seller Admin and Experience": (
        "S08UV261ZRU",
        "seller-seller-admin-and-experience",
    ),
    ...
    "BO Catalog - Master Experience": (
        "S08VD2Y1DMF",
        "catalog-bo-catalog-master-experience",
    ),
    ...
    "Navigation & Product discovery": (
        "S08UYHZFFD3",
        "marketplace-navigation-product-discovery",
    ),
    ...
    "Compromised laptop / server": (
        "S08UU3T6H51",
        "security-compromised-laptop-server",
    ),
    ...
    "Stolen account(s) or IT materials": (
        "S08UU3TFJ3D",
        "security-stolen-accounts-or-it-materials",
    ),
    ...
    "Traffic acquisition": (
        "S08UU3BNPPV",
        "marketing-communication-traffic-acquisition",
    ),
...
def update_components_slack_groups(apps, _):
    ...
    with transaction.atomic():
        for component_name, (
            user_group_id,
            user_group_name,
        ) in COMPONENT_SLACK_GROUPS.items():
            try:
                ...
                if not hasattr(user_group, "components"):
                    logger.error(
                        f"UserGroup '{user_group_name}' n'a pas d'attribut 'components'."
                    )
                    continue
                ...
                if not user_group.components.filter(id=component.id).exists():
                    ...
                    logger.info(
                        f"Component '{component_name}' ajouté au SlackUserGroup '{user_group_name}'"
                    )
            except Exception:
...
def ensure_firefighters_user_group(apps, _):
    try:
        ...
        logger.exception(
            "The 'slack' app is not installed. Skipping Firefighters user group check."
        )
        return
    ...
    for component in Component.objects.all():
        ...
        if not firefighters_group.components.filter(id=component.id).exists():
            ...
            logger.info(
                f"Component '{component.name}' ajouté au SlackUserGroup 'firefighters'."
            )
...
def cleanup_unused_slack_user_groups(apps, _):
    try:
        ...
        logger.exception(
            "The 'slack' app is not installed. Skipping user groups cleanup."
        )
        return
...
def delete_unused_slack_user_groups(apps, _):
    try:
        ...
        logger.exception(
            "The 'slack' app is not installed. Skipping user groups deletion."
        )
        return
    ...
    for user_group in UserGroup.objects.all():
        ...
        if hasattr(user_group, "components") and user_group.components.count() == 0:
            logger.info(
                f"Deleting SlackUserGroup '{user_group.name}' (ID: {user_group.usergroup_id}) as it has no components."
            )
            user_group.delete()
